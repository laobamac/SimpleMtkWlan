...
taskq_next_work(struct taskq *tq, struct task *work)
{
    ...
    //    IOLog("SimpleMtkWlan: taskq %s lock\n", __FUNCTION__);
    ...
    *work = *next; /* copy to caller to avoid races */
    ...
    IORecursiveLockUnlock(tq->tq_mtx);
//    IOLog("SimpleMtkWlan: taskq %s unlock\n", __FUNCTION__);
...
//    WITNESS_CHECKORDER(&tq->tq_lock_object, LOP_NEWORDER, NULL);
    ...
    IOLog("SimpleMtkWlan: taskq %s schedule task\n", __FUNCTION__);

    while (taskq_next_work(tq, &work)) {
//        WITNESS_LOCK(&tq->tq_lock_object, 0);
//        IOLog("SimpleMtkWlan: taskq worker thread=%lld work=%s\n", thread_tid(current_thread()), work.name);
        (*work.t_func)(work.t_arg);
//        IOLog("SimpleMtkWlan: taskq worker thread=%lld work=%s done", thread_tid(current_thread()), work.name);
//        WITNESS_UNLOCK(&tq->tq_lock_object, 0);
//        sched_pause(yield);
        IOSleep(1);
    }
    
    IOLog("SimpleMtkWlan: taskq %s schedule task done\n", __FUNCTION__);
...
//        KERNEL_LOCK();
    ...
    if (last) {
        IOLog("SimpleMtkWlan: taskq %s schedule task wakeup\n", __FUNCTION__);
...
void taskq_create_thread(void *arg)
{
    ...
    switch (tq->tq_state) {
        ...
        default:
            IOLog("SimpleMtkWlan: unexpected %s tq state %u", tq->tq_name, tq->tq_state);
            IORecursiveLockUnlock(tq->tq_mtx);
            if (tq != systq) {
                IORecursiveLockFree(tq->tq_mtx);
                IOFree(tq, sizeof(*tq));
    ...
    do {
        ...
        IORecursiveLockLock(tq->tq_mtx);
        if (rv != KERN_SUCCESS) {
            IOLog("SimpleMtkWlan: tasq unable to create thread for \"%s\" taskq\n",
                   tq->tq_name);
...
{
    int rv = 0;
//    IOLog("SimpleMtkWlan: taskq task_add %s thread: %lld\n", w->name, thread_tid(current_thread()));
    ...
    IORecursiveLockLock(tq->tq_mtx);
    if (ISSET(w->t_flags, TASK_ONQUEUE)) {
//        IOLog("SimpleMtkWlan: taskq task_add %s is already on queue thread: %lld\n", w->name, thread_tid(current_thread()));
        IORecursiveLockUnlock(tq->tq_mtx);
        return (0);
    }
    if (!ISSET(w->t_flags, TASK_ONQUEUE)) {
//        IOLog("SimpleMtkWlan: taskq task_add %s add to queue thread: %lld\n", w->name, thread_tid(current_thread()));
        rv = 1;
...
{
    int rv = 0;
//    IOLog("SimpleMtkWlan: taskq task_del %s thread: %lld\n", w->name, thread_tid(current_thread()));
